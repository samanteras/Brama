...
export function ChatPreview({ messages, host = 'yourcompany.com', className }: ChatPreviewProps) {
  return (
    <div
      className={cn(
        'overflow-hidden rounded-xl border bg-card shadow-xl shadow-black/5 dark:shadow-black/40',
        className,
      )}
    >
      ...
      <div className="space-y-4 p-4 sm:p-5">
        {messages.map((message, index) => (
          <div key={index} className="space-y-3">
            ...
            {message.showLeadForm ? <LeadFormPreview /> : null}
          </div>
        ))}
      </div>
    </div>
...
function LeadFormPreview() {
  return (
    <div className="ml-0 max-w-[85%] rounded-2xl rounded-bl-sm border border-dashed bg-background p-3">
      <div className="flex items-center gap-2">
        <span className="flex-1 rounded-md border bg-muted/40 px-3 py-2 text-sm text-muted-foreground">
          +48 601 234 567
        </span>
