...
/**
 * Pricing cards, rendered entirely from `lib/plans.ts` — in both languages.
 *
...
export function Pricing({
  ...
  locale: MarketingCopy['locale']
}) {
  const taglines = locale === 'ru' ? PLAN_TAGLINES_RU : PLAN_TAGLINES
  ...
  return (
    <section id="pricing" className="border-b">
      <div className="mx-auto w-full max-w-6xl px-4 py-24 sm:px-6 lg:py-32">
              ...
              <StaggerItem key={plan.id} className="flex">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {isFeatured ? <Badge>{copy.featuredBadge}</Badge> : null}
                  </div>
                  ...
                  <p className="mt-6 flex items-baseline gap-1.5">
                    <span className="text-5xl font-semibold tracking-tight">
                      {formatPrice(plan.monthlyPriceCents, locale)}
                    </span>
                  ...
                  >
                    <Link href="/sign-up">
                      {plan.monthlyPriceCents === 0 ? copy.startFree : copy.choosePlan(plan.name)}
                    </Link>
                  </Button>
                </Card>
              </StaggerItem>
        ...
        <Reveal delay={0.1}>
          <p className="mt-10 text-sm text-muted-foreground">{copy.footnote}</p>
        </Reveal>
      </div>
    </section>
  )
